...
export async function AppShell({ children }:{ children:React.ReactNode }) {
  ...
  return <div className="pokedeal-shell min-h-screen min-w-0 overflow-x-hidden md:grid md:grid-cols-[240px_minmax(0,1fr)]">
    <header className="mobile-header md:hidden"><Link href="/dashboard" className="brand-mark"><span>POKÉ</span>DEAL</Link><div className="topbar-actions">{isAdmin&&<Link href="/settings" className="coll-topbar-btn" aria-label="Configuration">⚙</Link>}<AccountControl compact /></div></header>

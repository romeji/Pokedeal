...
type Deal = { id:string; title:string; description?:string|null; url:string; imageUrl?:string|null; price:number; marketValue:number|null; profit:number|null; roi:number|null; score:number|null; category?:string|null; confidence:number|null; risk:number|null; listingStatus:string; country?:string|null; condition?:string|null; publishedAt:string; decision:string; productId:string|null; productName:string|null; matchConfidence:number|null };
...
export function DealLibrary() {
  ...
  return <main className="app-page maquette-page deals-page">
    ...
    <div className="deal-search-row"><input className="neu-input" value={q} onChange={(event)=>setQ(event.target.value)} onKeyDown={(event)=>{if(event.key==="Enter")void load(1)}} placeholder="Rechercher dans les annonces…"/><button className="circle-action" onClick={()=>void load(1)} aria-label="Rechercher">⌕</button><button className={`circle-action view-number ${view==="list"?"active":""}`} onClick={()=>setView("list")} aria-label="Une colonne">1</button><button className={`circle-action view-number ${view==="grid2"?"active":""}`} onClick={()=>setView("grid2")} aria-label="Deux colonnes">2</button><button className={`circle-action view-number ${view==="grid3"?"active":""}`} onClick={()=>setView("grid3")} aria-label="Trois colonnes">3</button></div>
    ...
    {busy?<div className="loading-panel">Analyse des deals…</div>:<section className={`deal-full-results ${view}`}>{rows.map((deal)=><DealCard key={deal.id} deal={deal} decide={decide} select={()=>setSelected(deal)}/>) }{!rows.length&&<div className="neu-card empty-inline">Aucun deal fiable ne correspond à ces filtres.</div>}</section>}
    ...
    {selected&&<div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)setSelected(null)}}><section className="app-modal deal-detail-modal"><button className="modal-close" onClick={()=>setSelected(null)}>×</button>{selected.imageUrl?<img src={selected.imageUrl} alt="" className="deal-detail-photo"/>:<div className="deal-detail-photo image-placeholder">🏷</div>}<span className="catalog-code">Vinted · {selected.country||"Pays inconnu"}</span><h2>{selected.title}</h2><p>{selected.description||"Aucune description fournie par le vendeur."}</p><div className="deal-stats"><div className="cell"><span>Achat</span><strong>{euro(selected.price)}</strong></div><div className="cell"><span>Marché</span><strong>{selected.marketValue===null?"À vérifier":euro(selected.marketValue)}</strong></div><div className="cell"><span>Profit prudent</span><strong className="gain">{selected.profit===null?"—":`+${euro(selected.profit)}`}</strong></div><div className="cell"><span>Confiance</span><strong>{selected.matchConfidence===null?"—":`${Math.round(selected.matchConfidence*100)}%`}</strong></div></div><p className="deal-price-warning">La cotation n'est affichée que pour les associations suffisamment fiables. Vérifie toujours les photos, la langue, l'état et la référence avant achat.</p><div className="deal-detail-links">{selected.productId&&<a href={`/items/${selected.productId}`} className="neu-button">Voir l'item dans PokéDeal</a>}<a href={selected.url} target="_blank" rel="noreferrer" className="button-primary">Ouvrir sur Vinted ↗</a></div></section></div>}
  </main>;
}

function DealCard({deal,decide,select}:{deal:Deal;decide:(id:string,status:string)=>Promise<void>;select:()=>void}){
  const scoreClass=(deal.score??0)>=80?"score-good":(deal.score??0)>=55?"score-mid":"score-low";
  const stop=(event:React.MouseEvent)=>event.stopPropagation();
  return <article role="button" tabIndex={0} onClick={select} onKeyDown={(event)=>{if(event.key==="Enter")select()}} className={`deal-full neu-card clickable ${deal.decision!=="TO_REVIEW"?"done":""}`}><div className="deal-full-top">{deal.imageUrl?<img className="thumb" src={deal.imageUrl} alt="" loading="lazy"/>:<div className="thumb">🏷</div>}<div className="info"><h4>{deal.title}</h4><div className="meta">Vinted · {relativeDate(deal.publishedAt)} · {deal.country||"pays inconnu"}</div><div className="deal-tags"><span>{deal.condition||"État inconnu"}</span><span>{dealVerificationLabel(deal.listingStatus)}</span></div></div>{deal.score!==null?<div className={`score-badge ${scoreClass}`}>{deal.score}<span>/100</span></div>:<div className="score-badge score-low">—</div>}</div><div className="deal-stats"><div className="cell"><span>Achat</span><strong>{euro(deal.price)}</strong></div><div className="cell"><span>Marché</span><strong>{deal.marketValue===null?"—":euro(deal.marketValue)}</strong></div><div className="cell"><span>Profit</span><strong className="gain">{deal.profit===null?"—":`+${euro(deal.profit)}`}</strong></div><div className="cell"><span>ROI</span><strong className="gain">{deal.roi===null?"—":`+${deal.roi.toFixed(0)}%`}</strong></div></div><div className="deal-full-actions" onClick={stop}><button className="button-primary" onClick={select}>Détails</button><button className={deal.decision==="VALIDATED"?"active":""} onClick={()=>void decide(deal.id,"VALIDATED")}>✓ Valider</button><button className={deal.decision==="BOUGHT"?"active":""} onClick={()=>void decide(deal.id,"BOUGHT")}>＋ Acheté</button><button className={deal.decision==="IGNORED"?"danger active":"danger"} onClick={()=>void decide(deal.id,"IGNORED")}>× Ignorer</button></div></article>;
}

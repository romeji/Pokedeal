...
export function BlocksExplorer() {
  ...
  return <main className="app-page maquette-page collection-explorer"><header className="maquette-topbar"><div><span>Ma collection</span><h1>{selectedSet?.name ?? selectedSeries?.name ?? (root ? "Collection" : "Blocs")}</h1></div><div className="topbar-actions"><button className={`circle-action ${showSearch?"active":""}`} onClick={()=>setShowSearch((value)=>!value)} aria-label="Rechercher">⌕</button><Link href="/portfolio" className="circle-action" aria-label="Créer un classeur">＋</Link></div></header>
    ...
    {busy ? <div className="loading-panel">Chargement du catalogue…</div> : root ? <section className="block-grid"><button className="block-tile neu-card" onClick={()=>setRoot(false)}><span className="ico">▦</span><strong>Blocs</strong><small>{series.length} blocs</small></button><Link className="block-tile neu-card" href="/portfolio"><span className="ico">▤</span><strong>Classeurs</strong><small>Gérer mes collections</small></Link><Link className="block-tile neu-card" href="/collection/wishlist"><span className="ico">☆</span><strong>Souhaits</strong><small>Liste à surveiller</small></Link></section> : selectedSet ? <section className="card-catalog-grid">{filteredCards.map((card)=><article key={card.id} className={`catalog-card ${card.owned ? "owned" : ""}`}><button className="catalog-card-main" onClick={() => setSelectedCard(card)}>{card.imageUrl ? <img src={card.imageUrl} alt={card.name} loading="lazy" /> : <div className="image-placeholder">◇</div>}<span className="catalog-card-number">#{card.number}</span><h2>{card.name}</h2></button><button className={`wish-button ${card.wished ? "active" : ""}`} onClick={() => void toggleWish(card)} aria-label={card.wished ? "Retirer de la liste de souhaits" : "Ajouter à la liste de souhaits"}>{card.wished ? "★" : "☆"}</button>{card.owned&&<span className="owned-badge">✓</span>}</article>)}</section> : selectedSeries ? <section className="set-grid">{filteredSets.map((set)=><button key={set.id} onClick={() => void openSet(set)} className="set-card neu-card"><div className="cover">{set.logo ? <img src={set.logo} alt={set.name} /> : "PK"}</div><span className="catalog-code">{alias(set.id)} · {year(set.releaseDate)}</span><h5>{set.name}</h5><div className="meta">{set.owned}/{set.total} cartes · {euro(set.value)}</div><div className="progress-track"><span style={{ width: `${Math.min(100, (set.owned / Math.max(1,set.total)) * 100)}%` }} /></div></button>)}</section> : <section className="bloc-grid">{filteredSeries.map((item)=><button key={item.id} onClick={() => void openSeries(item)} className="bloc-card neu-card"><div className="cover">{item.logo ? <img src={item.logo} alt={item.name} /> : "PK"}</div><h5>{item.name}</h5><div className="meta">{item.setCount} série{item.setCount > 1 ? "s" : ""} · {year(item.releaseDate)}</div></button>)}</section>}

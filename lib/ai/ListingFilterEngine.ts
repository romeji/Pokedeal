...
const BUILTIN = [
  ...
  { category:"PACKAGING_ONLY", action:"REJECT" as const, label:"Emballage seul", terms:["boite vide","boites vides","tin vide","mini tin vide","coffret vide","display vide","etb vide","booster vide","emballage seul","uniquement la boite","boite uniquement","boite seule","juste la boite","empty box","empty tin","empty etb","sans cartes","sans booster","sans contenu"] },
  { category:"OPENED", action:"REVIEW" as const, label:"Produit ouvert", terms:["deja ouvert","booster ouvert","display ouverte","coffret ouvert","produit ouvert","descele","non scelle","loose pack","contenu incomplet","sans blister"] },
];
...
export async function evaluateListing(title:string,description?:string|null):Promise<FilterDecision>{
  ...
  const packagingTokens=["boite","tin","etb","coffret","display","booster","blister","emballage","box","pack"];
  const emptyTokens=["vide","vides","empty","sans cartes","sans booster","sans contenu","boite seule"];
  if(packagingTokens.some((term)=>text.includes(term))&&emptyTokens.some((term)=>text.includes(term))){
    flags.push("PACKAGING_ONLY");reasons.push("Emballage seul");action="REJECT";
  }

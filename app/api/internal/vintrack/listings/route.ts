...
export async function POST(request: Request) {
  ...
  for (const raw of body.listings) {
    ...
    const sellerCountry = raw.sellerCountry || inferCountry(raw.url);
    const listing = await prisma.listing.upsert({
      where: { marketplace_externalId: { marketplace: "vinted", externalId: String(raw.externalId) } },
      update: { url:raw.url, title:raw.title, description:raw.description??null, price:raw.price, currency:raw.currency||"EUR", sellerCountry:sellerCountry||undefined, itemCondition:raw.itemCondition||undefined, publishedAt, lastSeenAt:new Date(), listingHash:computeListingHash(raw.title,raw.price), titleHash:computeTitleHash(raw.title) },
      create: { marketplace:"vinted", externalId:String(raw.externalId), url:raw.url, title:raw.title, description:raw.description??null, price:raw.price, currency:raw.currency||"EUR", sellerCountry:sellerCountry||null, itemCondition:raw.itemCondition||null, publishedAt, status:"NEW", listingHash:computeListingHash(raw.title,raw.price), titleHash:computeTitleHash(raw.title) },
    });
...
function inferCountry(value: string) {
  try {
    ...
    return ({ "www.vinted.fr":"FR", "www.vinted.be":"BE", "www.vinted.de":"DE", "www.vinted.es":"ES", "www.vinted.it":"IT", "www.vinted.nl":"NL", "www.vinted.pt":"PT", "www.vinted.lu":"LU" } as Record<string,string>)[host] ?? null;
  } catch { return null; }
}
